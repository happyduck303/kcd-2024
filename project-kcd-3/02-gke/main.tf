terraform {
  ...
  backend "gcs" {
    bucket = "kcd-2024"
    prefix = "project-kcd-3/gke/prome-secondary"
  }
}

locals {
  project = "project-kcd-3"
...
resource "google_service_account" "gke-prome-secondary" {
  account_id   = "gke-prome-secondary"
  display_name = "gke-prome-secondary"
  description  = "used by gke prome-secondary"
}

resource "google_project_iam_member" "gke-prome-secondary" {
  for_each = toset([
  ...
  project = local.project
  role    = each.value
  member  = "serviceAccount:${google_service_account.gke-prome-secondary.email}"
}
